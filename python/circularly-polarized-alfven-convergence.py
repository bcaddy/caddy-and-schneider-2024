...
================================================================================
 ...
 Generates the circularly polarized alfven wave convergence plot.
================================================================================
"""
...
waves          = ['standing', 'moving']
speed          = {'standing':0, 'moving':-1}
resolutions    = [16, 32, 64, 128, 256, 512]

# L2 Norm data save path
save_path = shared_tools.repo_root / 'python' / 'cpaw_l1_norms.pkl'
...
def main():
    ...
    parser.add_argument('-l', '--l2_norms', action="store_true", help='Compute the L2 Norms')
        ...
        for wave in waves:
            for resolution in resolutions:
                shared_tools.cholla_runner(reconstructor=reconstructor,
                                           param_file_name=f'circularly_polarized_alfven_wave.txt',
                                           cholla_cli_args=f'nx={resolution} ny={resolution//2} nz={resolution//2}',
                                           move_initial=True,
                                           move_final=True,
                                           initial_filename=f'cpaw_{reconstructor}_{wave}_{resolution}_initial',
                                           final_filename=f'cpaw_{reconstructor}_{wave}_{resolution}_final')

                # Print status
                print(f'Finished with {resolution}, {wave} CPAW, {reconstructor}')
# ==============================================================================
...
def computeL2Norm():
    ...
    for reconstructor in reconstructors:
        for wave in waves:
            for resolution in resolutions:
                ...
                initial_data = shared_tools.load_conserved_data(f'cpaw_{reconstructor}_{wave}_{resolution}_initial')
                final_data   = shared_tools.load_conserved_data(f'cpaw_{reconstructor}_{wave}_{resolution}_final')
    ...
    shared_tools.pickle_dictionary(l2_data, save_path)
# ==============================================================================

# ==============================================================================
def plotL2Norm(outPath, normalize = False):
    # Plotting info
    data_linestyle     = '-'
    linewidth          = 1
    plmc_marker        = '.'
    ...
    # Load the L2 Norms data
    L2Norms = shared_tools.unpickle_dictionary(save_path)
