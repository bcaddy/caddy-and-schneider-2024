...
The induction equation requires that the magnetic field be divergence free, i.e. the Universe does not contain magnetic monopoles. However, \textbf{achieving this numerical has turned out to be complex and several solutions have been used in different combinations \citep{dedner_hyperbolic_2002,mignone_2010,Powell1997, rossmanith_2006,Crockett_2005}. % mention other methods, brief facts.
We chose to use Constrained Transport (CT) due to its accuracy and because it is well suited static structured grid codes like Cholla % also because CT is expensive and GPUs are fast. 
